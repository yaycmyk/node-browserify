...
var config = require(process.cwd() + '/package.json').browserify;
...
module.exports = function (args, opts) {
    ...
    [].concat(argv.p).concat(argv.plugin, config.plugin.map(wrap_plugin_definition)).filter(Boolean)
        .forEach(function (p) {
            var pf = p, pOpts = {};
            if (typeof p === 'object') {
    ...
    if (config.plugin) {
        config.plugin.forEach(function(plugin_def) {
            var plugin_func = require(plugin_def[0]);
            var plugin_opts = plugin_def[1];

            if (typeof plugin === 'function') {
                b.plugin(plugin_func, plugin_opts);
            }
        });
    }
    ...
    [].concat(argv.transform, config.transform)
        .filter(Boolean)
